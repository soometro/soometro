...
| 혈정석 종류 및 세팅 | 1옵션 수치 | 2옵션 수치 | 개당 상승률 | 혈정석 3개 총 효율 | 최종 표기 공격력 |
|---|---|---|---|---|---|
| 뚱땡이(삼뚱) 3개 장착 | 1.272 | 1 | 1.2720 | 2.06 | 502.2 |
| 야피주 2개 + 뚱땡이 1개 | 1.248 | 1.057 | 1.3191 | 2.21 | 540.1 |
| 광녀 2개 + 뚱땡이 1개 | 1.210 | 1.120 | 1.3552 | 2.34 | 570.0 |
| 광녀 3개 장착 | 1.210 | 1.120 | 1.3552 | 2.49 | 607.3 |
| 빈사유령 3개 장착 | 1.091 | 1.270 | 1.3856 | 2.66 | 649.0 |
| 가난한 자 3개 장착 | 1.122 | 1.270 | 1.4249 | 2.89 | 706.0 |
...
야수 피의 주인(야피주) 혈정석 옵션은 1옵션 물리 24.8% 상승에 2옵션 HP 최대 시 공격력 5.7% 상승으로 계산을 진행한다.
그러나 두 번째 옵션에 % 증가가 아니라 '물리 공격력 +15'와 같은 가산 옵션이 붙는 경우가 있다. 이를 계산하는 방식은 다음과 같다.

* 승산(곱연산) 계산: 244 * 1.248 * 1.248 * 1.272 = 483.4
* 가산(더하기) 계산: +15 + 15 = +30
* 최종 합산 결과: 483.4 + 30 = 513.4

표에 기록된 540.1보다 수치상으로는 적은 513.4가 나오지만, 데미지 +30을 붙이는 것은 조건 없이 확정적으로 적용되기 때문에 실전에서는 가산 옵션이 더 좋다고 말할 수 있다.  

보스전 도중 적에게 한 대 맞아서 캐릭터의 피통(HP)이 깎였을 때를 가정해보자. HP 최대 시 발동하는 옵션이 꺼진 상태의 야피주 세팅 공격력을 계산해보면 483.4(원문 표기 약 480) 수준으로 떨어진다.  

2옵션 선택은 유저 개개인의 플레이 스타일과 취향의 문제이므로 어떤 것이 절대적으로 더 낫다고 단정할 수는 없다. 하지만 리스크 없는 상시 데미지와 조건부 맥스 데미지의 중간값 정도를 안정적으로 보장하는 513.4 세팅이 실전에서 더 유용하지 않을까 생각한다.
